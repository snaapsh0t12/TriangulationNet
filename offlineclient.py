"""
The code for the client nodes, an offline version that records to a file instead of sending to the server
    ...
    while True:
        ...
        if not networks:
            ...
            for ssid, signal in networks:
                ...
                timeof = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                print(timeof)
                with open("data/wifi_log.txt", "a") as file:
                    file.write(f"{timeof} - SSID: {ssid}, Mac Address: {mac_address}, Signal Strength: {signal}%\n")
                
                with open("data/wifi_log.csv", "a") as file:
                    file.write(f"{timeof},{ssid},{mac_address},{signal}\n")
